...
public class NoSQLStreamExample {
    ...
    /* subscription time out in milliseconds */
    private static final int SUBSCRIPTION_TIMEOUT_MS = 10000;
    ...
    private void doSubscribe() throws Exception {
        ...
        try {
            ...
            /* wait for subscription to succeed or timeout */
            waitFor(new ExamplePollCondition(SUBSCRIPTION_TIMEOUT_MS) {
                @Override
             ...
             */
            waitFor(new ExamplePollCondition(MAX_SUBSCRIPTION_TIME_MS) {
                @Override
                protected boolean condition() {
                    return subscriber.getStreamOps() == num;
                }
            });

            if (subscriber.isCkptSucc()) {
                logger.info("Checkpoint succeeded");
            } else {
                logger.info("No checkpoint or checkpoint failed");
            }
    ...
    protected void waitFor(final ExamplePollCondition examplePollCondition)
        throws TimeoutException {

        boolean success = examplePollCondition.await();
        /* if timeout */
        if (!success) {
    ...
    /**
     * A polling object that periodically checks if the condition is met or
     * timeout.
     */
    private abstract class ExamplePollCondition {
        ...
        ExamplePollCondition(long timeout) {
